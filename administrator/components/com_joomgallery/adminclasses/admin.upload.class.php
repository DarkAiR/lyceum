...
// $Id: admin.upload.class.php 457 2009-07-26 10:38:28Z aha $
/******************************************************************************\
**   JoomGallery  1.5                                                         **
...
class Joom_Upload 
{
  ...
  function Upload_Batch($catid) 
  {
    ...
    for ($i=0; $i < $sizeofzip; $i++) 
    {
      ...
      //Set permissions of original image to 644
      ...
      //Create the thumbnail from original picture
      ...
      //Set permissions of thumbnail to 644
      Joom_Chmod(JPath::clean(JPATH_ROOT.DS.$config->jg_paththumbs.$catpath.$newfilename));     
      ...
      //Set permissions of detail to 644
      Joom_Chmod(JPath::clean(JPATH_ROOT.DS.$config->jg_pathimages.$catpath.$newfilename));            
  ...
  */
  function Upload_FTP ($catid) 
  {
    ...
    foreach ($this->ftpfiles as $screenshot_name) 
    {
      ...
      if(!($config->jg_delete_original == 1 
         || ($config->jg_delete_original== 2 && $this->original_delete==1))) 
      {
        ...
        //Set permissions of original to 644        
        Joom_Chmod(JPath::clean(JPATH_ROOT.DS.$config->jg_pathoriginalimages.$catpath.$newfilename));       
      } 
      else 
      {
  ...
  function Upload_AppletReceive_Backend ($catid) 
  {
    ...
    foreach ($_FILES as $file => $fileArray) 
    {
      ...
      if (strlen($screenshot) > 0 && $screenshot != 'none') 
      {
        ...
        //Set permissions of uploaded file to 644                  
